...
// 左侧菜单栏数据
const menu = ref([
  { path: "/educational/e/601", meta: "教务信息" },
  { path: "/educational/e/602", meta: "教学研讨" },
]);
  ...
  (toRoute, fromRoute) => {
    ...
    menu.value.forEach((item) => {
      ...
      if (match && match[1] === articleType.toString()) {
        toRoute.matched[2].path = "/educational/e/" + articleType;
        toRoute.matched[2].meta.title = item.meta;
      }
    });

    // 通常，我们只需要toRoute.matched，因为它包含了当前路由及其所有父路由的信息
    toRoute.matched[1].path = "/educational/e/601"; // 手动修改第2层路由为，默认路由
